...
// Turn off error reporting to avoid displaying any error messages to the user
error_reporting(0);

// Include the database connection file
include_once('includes/dbconnection.php');

// Check if the form with the 'addcart' button has been submitted
if (isset($_POST['addcart'])) {
    // Retrieve the food ID, food quantity, and user ID from the submitted form
    $foodid = $_POST['foodid'];
    $foodqty = $_POST['foodqty'];
    $userid = $_SESSION['fosuid'];

    // Insert the user ID, food ID, and food quantity into the 'tblorders' table in the database
    $query = mysqli_query($con, "INSERT INTO tblorders (UserId, FoodId, FoodQty) VALUES ('$userid', '$foodid', '$foodqty')");

    // Check if the query is successful
    if ($query) {
        // Display an alert to the user that the food has been added to the cart
        echo "<script>alert('Food has been added in to the cart');</script>";
    } else {
        // If the query is not successful, display an alert to the user that something went wrong
